...
# -*- coding: utf-8 -*-
"""
usage: list_users.py [-h] [-f CONFIG_FILE] --app-uri APP_URI --oidc-uri OIDC_URI [--realm REALM]
                     ...
                     [--search SEARCH] [-o OUTPUT]

Fetch users from GCM and print the result as JSON.
...
options:
  ...
  --search SEARCH       Search string (optional)
  -o OUTPUT, --output OUTPUT
                        Output JSON file path (default: users.json)
"""
...
def build_arg_parser():
    ...
    parser.add_argument("--search", default="", 
                        help="Search string (optional)")

    # Output file path
    parser.add_argument("-o", "--output", default="users.json",
                        help="Output JSON file path (default: users.json)")
    return parser
...
def main(argv):
    ...
    # Call users listing API
    users_path = "ibm/usermanagement/api/v1/users"
    try:
        print(f"\nFetching users from: {config['app_uri']}/{users_path}")
        ...
        print(f"Users API Response: HTTP {resp.status_code}")
    except Exception as e:
    ...
    try:
        ...
        if isinstance(payload, dict):
            ...
            print(f"\nSummary: Retrieved {len(content)} users (Total: {total})")
    except Exception:
